...
.hasTable('devices').then(function(exists) {
    if (!exists) {
      return knex.schema.createTable('devices', function(t) {
        ...
        t.integer('baurate').nullable; // var dataGDAQ = {barocde: "", massweigh:"", calweigh, priceweigh, diffweigh, rate, length,width, height, massweighVNP, calweighVNP, priceweighVNP, lengthVNP, widthVNP, heightVNP}
        t.integer('databits').nullable;
        t.integer('stopbits').nullable;
